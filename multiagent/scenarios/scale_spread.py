...
class Scenario(BaseScenario):
    def make_world(self, **kwargs):
        ...
        # number of observable entities 
        world.obs_agents = kwargs.get("obs_agents", 3)
        world.obs_landmarks = kwargs.get("obs_landmarks", 4)
    ...
    def info(self, agent, world):
        # sorted agent indices based on ditance to current agent 
        temp = []
        for i, a in enumerate(world.agents):
            if a is agent: continue
            d = np.square(a.state.p_pos - agent.state.p_pos)
            temp.append([i, d])
        indices = [e[0] for e in sorted(temp, key=lambda x: x[-1])]
        dists = [e[1] for e in sorted(temp, key=lambda x: x[-1])]
        # collect info to return 
        return {
            "sorted_agents": indices,
            "sorted_distances": dists
        }